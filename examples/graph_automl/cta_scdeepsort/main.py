...
if __name__ == "__main__":
    ...
    parser.add_argument("--dense_dim", type=int, default=400, help="placeholder in automl")
    ...
    def evaluate_pipeline(tune_mode=args.tune_mode, pipeline_planer=pipeline_planer):
        ...
        for run_idx in range(args.num_runs):
            ...
            run_train_score = model.score(g_train, y_train)
            run_valid_score = model.score(g_valid, y_valid)
            run_test_score = model.score(g_test, y_test)
            ...
            del model, data, g, g_train, g_valid, g_test
            gc.collect()
            if args.device != "cpu": torch.cuda.empty_cache()
