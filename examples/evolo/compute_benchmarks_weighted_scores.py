"""
Compute weighted scores from *benchmarks_openevolve_output results.

For each run i in a results_*.json file:
    weighted_score_i = scores[i] * 0.8 + 0.2 * (1 / (1 + 2 * times[i] / 300))

Outputs one CSV per dataset to ./weighted_scores_benchmarks/.
...
OUTPUT_DIR = EVOLO_DIR / "weighted_scores_benchmarks"
...
def extract_algorithm(bench_dir: Path) -> str:
    """'graphcs_benchmarks_openevolve_output' -> 'graphcs'"""
    return bench_dir.name.replace("_benchmarks_openevolve_output", "")
...
def extract_dataset(filename: str, algorithm: str) -> str:
    ...
    e.g. results_human_CD8_...json  (algorithm=graphcs)  -> human_CD8_...
         results_louvain_151507.json (algorithm=louvain)  -> 151507
    """
...
for bench_dir in sorted(EVOLO_DIR.rglob("*benchmarks_openevolve_output")):
    # Skip anything inside an 'old' folder
    if "old" in bench_dir.parts:
        continue
    ...
    algorithm = extract_algorithm(bench_dir)
...
OUTPUT_DIR_V2 = EVOLO_DIR / "weighted_scores_benchmarks_v2"
