...
class scGATGraphTransform(BaseTransform):
    ...
    _DISPLAY_ATTRS: Tuple[str] = ("label_column", "n_neighbors", "use_pca_features", "n_components", "use_highly_variable_genes")
    ...
    def __call__(self, data) -> Any:
        ...
        # 1. Preprocess features for better GAT performance
        self.logger.info("Preprocessing features...")
        
        # Select highly variable genes if requested and available
        if self.use_highly_variable_genes and 'highly_variable' in adata.var:
            self.logger.info("Using highly variable genes for feature extraction...")
            adata_subset = adata[:, adata.var['highly_variable']]
        else:
            adata_subset = adata

        if self.use_pca_features:
            # Use PCA features if available, otherwise compute them
            if 'X_pca' not in adata_subset.obsm:
                self.logger.info("Computing PCA features...")
                # Log1p transform and scale first
                adata_proc = adata_subset.copy()
                sc.pp.log1p(adata_proc)
                sc.pp.scale(adata_proc, zero_center=True)
                
                # Compute PCA with safety checks
                n_comps = min(self.n_components, min(adata_proc.shape) - 1)
                if n_comps <= 0:
                    n_comps = min(adata_proc.shape) - 1
                    if n_comps <= 0:
                        n_comps = 1  # Fallback to minimum 1 component
                        
                pca = PCA(n_components=n_comps)
                pca_features = pca.fit_transform(adata_proc.X)
                adata_subset.obsm['X_pca'] = pca_features
                self.logger.info(f"Computed PCA features with {pca.n_components_} components (explained variance ratio: {pca.explained_variance_ratio_.sum():.3f})")
            
            # Use PCA features (already standardized)
            features = adata_subset.obsm['X_pca'].copy()
        else:
            # Use log1p transformed and scaled gene expression
            adata_proc = adata_subset.copy()
            sc.pp.log1p(adata_proc)
            sc.pp.scale(adata_proc, zero_center=True)
            features = adata_proc.X
            
        # Ensure features are in proper numpy format
        ...
        # Only normalize if not using PCA features (PCA already provides standardized features)
        ...
        if 'neighbors' not in adata_subset.uns:
            ...
            # Use PCA if available, otherwise raw features
            use_rep = 'X_pca' if self.use_pca_features and 'X_pca' in adata_subset.obsm else 'X'
            sc.pp.neighbors(adata_subset, n_neighbors=self.n_neighbors, use_rep=use_rep)

        # 3. Process Adjacency Matrix with pruning
        self.logger.info("Processing adjacency matrix...")
        if 'connectivities' in adata_subset.uns['neighbors']:
            adj = adata_subset.uns['neighbors']['connectivities']
        elif 'connectivities_key' in adata_subset.uns['neighbors']:
            adj_key = adata_subset.uns['neighbors']['connectivities_key']
            adj = adata_subset.obsp[adj_key]
        else:
            raise ValueError("Could not find connectivities matrix in neighbors")

        # Apply edge weight thresholding if specified
        if self.edge_weight_threshold > 0:
            self.logger.info(f"Applying edge weight threshold: {self.edge_weight_threshold}")
            original_nnz = adj.nnz
            adj.data[adj.data < self.edge_weight_threshold] = 0
            adj.eliminate_zeros()
            new_nnz = adj.nnz
            self.logger.info(f"Pruned {original_nnz - new_nnz} edges based on threshold {self.edge_weight_threshold}")

        # Add self-loops (A_hat = A + I) - more efficient implementation
        adj = adj + sparse.eye(adata_subset.shape[0]).tocsr()
        ...
        # Consistently check both original adata and subset for labels
        if self.label_column in adata.obs.columns:
        ...
        elif self.label_column in adata_subset.obsm:
            self.logger.info(f"Found '{self.label_column}' in adata_subset.obsm (One-Hot format).")
            y_matrix = adata_subset.obsm[self.label_column]
        ...
        elif self.label_column in adata.obsm:
            # Also check the original adata for label consistency
            self.logger.info(f"Found '{self.label_column}' in original adata.obsm (One-Hot format).")
            ...
            raise ValueError(f"Label '{self.label_column}' not found in adata.obs or adata.obsm or adata_subset.obsm")
        ...
        if self.train_indices is not None or self.val_indices is not None or self.test_indices is not None:
            ...
            # Random stratified split with fixed ratios
            train_ratio = 0.8
            val_ratio = 0.1
            ...
            if remaining_ratio > 0:
                val_relative_size = val_ratio / remaining_ratio
        ...
        # 6. Convert to PyTorch Geometric Data (using efficient conversion)
