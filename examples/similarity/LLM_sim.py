    ...
    **Output Requirement:**
    ...
    try:
        response = client.chat.completions.create(
            model=MODEL_NAME,
            messages=[
                {"role": "system", "content": "You are a rigorous code similarity assessment system. Always output in valid JSON format."},
        ...
        weighted_sum = (
            (data.get('algo_score', 0) * 0.4) + 
            (data.get('structure_score', 0) * 0.3) + 
            (data.get('goal_score', 0) * 0.2) + 
            (data.get('syntax_score', 0) * 0.1)
        )
